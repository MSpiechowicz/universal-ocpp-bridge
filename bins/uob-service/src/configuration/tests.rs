use super::{
    ConfigurationLoadError, EMS_SCADA_PROFILE, FileConfiguration, MQTT_TARGET_KIND, validate,
};
...
fn the_ems_scada_preset_selects_one_mqtt_target_and_no_second_listener() {
    ...
    assert_eq!(selection.catalog.kind.as_str(), MQTT_TARGET_KIND);
    assert_eq!(
        validated.service.targets.len(),
        1,
        "the preset must not register a second implemented target"
    );
    assert!(
        validated
            .service
            ...
            .all(|entry| entry.kind.as_str() == MQTT_TARGET_KIND),
        "no EMS HTTP listener kind is implemented by this preset"
    );
    assert!(
        selection
            .catalog

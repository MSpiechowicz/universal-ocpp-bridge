...
use uob_protocol_adapter::{
    OutboundCall, SessionCallOutcome, v201::remote_control::RemoteControlSession,
};
...
async fn invalid_expired_unavailable_and_unprivileged_commands_never_reach_wire() {
    ...
    let mut cases = vec![
        ...
        (
            command(
                &snapshot,
                "extra",
                protocol("Reset", json!({"type":"Immediate","extra":"ignored?"})),
            ),
            CommandErrorCode::InvalidParameters,
        ),
        (
            command(
                &snapshot,
                "wrong-connector",
                protocol("UnlockConnector", json!({"evseId":1,"connectorId":2})),
            ),
            CommandErrorCode::InvalidParameters,
        ),
        (
            command(
                &snapshot,
                "zero",
                protocol("UnlockConnector", json!({"evseId":1,"connectorId":0})),
            ),
            CommandErrorCode::InvalidParameters,
        ),
        (
    ...
    spoofed.request.resource.native_protocol_reference = Some(NativeProtocolReference::Ocpp201 {
        evse_id: 1,
        connector_id: None,
    });
...
async fn snapshot_identity_epoch_and_protocol_are_bound_to_the_actual_socket() {
    ...
    let other = session("ocpp1.6", Duration::from_secs(1)).await;
    let identity = Arc::new(
        uob_protocol_adapter::v201::remote_control::LocalRemoteStartIdentity::new(
            vec![],
            &uob_provider_adapter::LocalChargingIdentityProvider,
            auth,
        )
        .await
        .unwrap(),

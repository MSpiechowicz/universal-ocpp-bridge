...
use uob_application::{CommandClock, FlowDiagnostics};
use uob_contracts::{Connectivity, Environment, ProtocolEdition, UtcTimestamp};
use uob_protocol_adapter::{
    CallSessionConfiguration, spawn_call_session, v16::data_transfer::complete_data_transfer,
};
type Peer =
...
async fn delayed_wire_reply_reconnect_and_diagnostics_preserve_boundaries() {
    ...
    for epoch in 0..2 {
        let (mut peer, _) = endpoint_support::connect_plain(address, "alpha", SECRET, "ocpp1.6")
            .await
            .unwrap();
    ...
    while let Some(record) = traces.read_after(after).unwrap().record {
        ...
        assert!(!encoded.contains("secret://fixture/unknown-payload"));
...
async fn exchange(
    ...
    store: &Store,
    registry: &uob_application::data_transfer::Registry,
    ...
    // Same message ID on a new connection is a new invocation, not an automatic replay.
    ...
    assert_eq!(
        ...
        json!([3,"fixture-16-transfer",{"status":"Accepted","data":"pong"}])
    );
    // Unknown vendor payloads never enter default flow records.
    peer.send(Message::Text(json!([2,"unknown","DataTransfer",{"vendorId":"unregistered","data":"secret://fixture/unknown-payload"}]).to_string().into())).await.unwrap();
    let incoming = timeout(TEST_BOUND, outputs.incoming.receive())
        .await
        .unwrap()
        .unwrap();

...
impl RemoteStartIdentity for Identity {
    fn authorized_token(
        ...
        _: &ResourceRef,
        _: UtcTimestamp,
    ) -> Option<charging_identity::PresentedChargingIdentity> {
        None
    }
}
    ...
    ExternalCommand::authenticated(
        CommandRequest {
            ...
            operation: protocol(
                "ChangeAvailability",
                if connector == 0 {
                    json!({"operationalStatus":kind})
                } else {
                    json!({"evse":{"id":1,"connectorId":connector},"operationalStatus":kind})
                },
            ),
...
pub async fn status(
    ...
    frame[3]["connectorStatus"] = json!(state);
    ...
    let response =
        v201::availability::complete_status(incoming.call, store, snapshot, context, Clock.now())
            .await
            .unwrap();

...
use uob_contracts::*;
use uob_protocol_adapter::v16::availability::observed_effect;
...
async fn scheduled_transaction_waits_for_completion_and_fresh_status_evidence() {
    ...
    let fixture_snapshot: StationSnapshot = serde_json::from_slice(include_bytes!(
        "../../../../crates/contracts/tests/fixtures/station-snapshot-ocpp16-v1.json"
    ))
    .unwrap();
    let mut transaction = fixture_snapshot.transactions[0].clone();
    transaction.resource = snapshot.resources[0].resource.clone();
    ...
    let durable = store
        ...
        .unwrap()
        .unwrap();
    status(&mut running, &store, &mut snapshot, 1, "Charging", 1).await;
...
async fn station_scope_waits_for_every_connector_and_never_widens_connector_control() {
    ...
    let store = open(&database);
    let (mut snapshot, port, coordinator) = setup(&store, running.handle.clone()).await;
    // Independent two-connector topology keeps controller zero outside charging resources.
    ...
    let durable = store
        ...
        .unwrap()
        .unwrap();
    status(&mut running, &store, &mut snapshot, 0, "Unavailable", 1).await;
    assert!(observed_effect(&durable, &event(&store, &snapshot).await).is_none());
    assert_eq!(
    ...
    let frame = json!([2,"stale","StatusNotification",{"connectorId":2,"status":"Available","errorCode":"NoError","timestamp":"2020-01-01T00:00:00Z"}]);
    let call = uob_protocol_adapter::v16::decode_call(frame.to_string().as_bytes()).unwrap();
    let context = context(&snapshot, 4);
    uob_protocol_adapter::v16::availability::complete_status(
        call,
        &store,
        &mut snapshot,
        context,
...
async fn delayed_reply_keeps_status_processing_live_and_operative_evidence_separate() {
    ...
    // Reading stays live while waiting for the charger response. Charging is also operative.
    status(&mut running, &store, &mut snapshot, 1, "Charging", 2).await;
    assert!(!submit.is_finished());
    running
        .peer
